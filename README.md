...
**No LLM.** Keyword lists per language plus a regex for order references.
The Gemini version hit a 20-request-per-day free-tier wall, so this reads the
same 900 messages for free, instantly, identically on every run — and eval.py
prices exactly what that costs in accuracy.
...
`data/` and `store/` are committed, so the app runs straight from a clone.
The app computes nothing at demo time — it reads pre-written CSVs.
...
- First response 3.2 → 1.9 hrs · CSAT 3.74 → 3.99 (**modelled**, and labelled as such)
- Health-complaint recall **100%**, 0 auto-answered — and still 0 with the
  safety gate deleted, because "Health complaint" has no row in
  `approved_answers.csv` in any language. The guardrail is a missing row.
- Topic accuracy 94.8% overall — English 100%, Malay 83%. That 17-point gap
  is the priced case for a language model.
